 ...
 */
boost::shared_ptr<TunnelStream> Relay::openStream(std::string &nextHost, uint16_t nextPort,
        nodeType relay_type, std::string &endHost, uint16_t endPort,
    ...
    if (error)
    {
        ...
        *err = error::host_not_found;
        //throw boost::system::system_error(error);
        boost::shared_ptr<TunnelStream> stream;
        return stream;
    }

    //check if it is the end relay
    if(relay_type == lastHop)
    {
    ...
    else
    {
        ...
        uint16_t socksId  = Util::getRandomId();
        boost::shared_ptr<SocksStream> sockStream (new SocksStream(io_service, socket, endHost, endPort, relay_type, socksId));
