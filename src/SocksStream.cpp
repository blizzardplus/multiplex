...
SocksStream::SocksStream(boost::asio::io_service &io_service, boost::shared_ptr<ip::tcp::socket> socket,
  ...
  : ioService(io_service), socket(socket), host(nextHost), port(nextPort), relayType(relay_type)
{
    streamId = socksId;
#ifdef DEBUG_STREAMID
    std::cerr << " Created Socks ID ======> " << streamId << "\t" << std::endl;
#endif
}
...
void SocksStream::sendConnect() {
    ...
    std::cerr << socket->local_endpoint().address().to_string() << ":" << socket->local_endpoint().port() << std::endl;
